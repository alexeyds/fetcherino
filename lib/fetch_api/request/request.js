...
export default class Request {
  constructor(url, options={}) {
    ...
    headers = this.initHeadersAndImplementBody({headers, body});
  ...
  initHeadersAndImplementBody({body, headers: headersLike}) {
    let headers = new Headers(headersLike);
    let contentType = headers.get("content-type");
    let { guessedContentType } = implementBody.call(this, {body: body, contentType});

    if (guessedContentType && !contentType) {
